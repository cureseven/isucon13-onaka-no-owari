...
func searchLivestreamsHandler(c echo.Context) error {
	...
	if c.QueryParam("tag") != "" {
		...
		tagId, found := tagCache.GetTagIDByName(keyTagName)
		if !found {
			return echo.NewHTTPError(http.StatusInternalServerError, "failed to get tags: "+err.Error())
		}

		tagIDList = append(tagIDList, int(tagId))
...
func fillLivestreamResponse(ctx context.Context, tx *sqlx.Tx, livestreamModel LivestreamModel) (Livestream, error) {
	...
	for i := range livestreamTagModels {
		tagModel, found := tagCache.GetTagByID(livestreamTagModels[i].TagID)
		if !found {
...
func fillLivestreamResponseFromModel(ctx context.Context, tx *sqlx.Tx, livestreamModel LivestreamModel) (Livestream, error) {
	ownerModel := UserModel{}
	if err := tx.GetContext(ctx, &ownerModel, "SELECT * FROM users WHERE id = ?", livestreamModel.UserID); err != nil {
		return Livestream{}, err
	}
